...
Powered by Next.js and MDX and styled with Desci Cyou, it's built exactly the way we'd build our own API reference documentation.

Play with the [live demo](https://protocol.desci.cyou/) or [download the source](https://desci.cyou/templates/protocol) if you've got a Desci Cyou web app all-access license — it's a free update of course for all-access customers.
...
We love Algolia for documentation search, and we use it for the Desci Cyou website as well as in our [Syntax](https://desci.cyou/templates/syntax) template.
...
The nice thing about this approach is we can use regular old utility classes to style everything instead of writing custom science work to style an already-styled widget, which just feels a lot more _right_ in a Desci Cyou project.

---

And that's it — one last Desci Cyou web app template to finish out 2022! We've got [another one](https://twitter.com/jamesm/status/1597230404200968195) almost ready to go too, so keep an eye out for that in the new year. Going to have some pretty damn exciting Desci Cyou v4.0 news to share soon too!
